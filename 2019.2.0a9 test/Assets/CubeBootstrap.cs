...
public class CubeBootstrap : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        EntityArchetype archeType = mgr.CreateArchetype(typeof(RenderMesh), typeof(Translation), typeof(Rotation), typeof(LocalToWorld));
        ...
        mgr.SetComponentData(cube, new Translation { Value = spawnPos });
        mgr.SetSharedComponentData(cube, shared);

        cube = mgr.CreateEntity(archeType);
        mgr.SetComponentData(cube, new Translation { Value = spawnPos + new Vector3(0, 3f, 0) });
